...
public class BeerService {
	...
	public BeerDTO decrement(long id, Integer quantityDecrement) throws BeerStockExceededException, BeerNotFoundException {
		Beer beerToIncrementStock = verifyIfExists(id);
		int quantityAfterIncrement = quantityDecrement - beerToIncrementStock.getQuantity();
		if (quantityAfterIncrement <= beerToIncrementStock.getMax()) {
			beerToIncrementStock.setQuantity(beerToIncrementStock.getQuantity() - quantityDecrement);
			Beer incrementedBeerStock = beerRepository.save(beerToIncrementStock);
			return beerMapper.toDTO(incrementedBeerStock);
		}
		throw new BeerStockExceededException(id, quantityDecrement);
	}
}
